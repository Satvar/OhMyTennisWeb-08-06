...
export class AdmincoachlistComponent extends AdminComponent implements OnInit {
  ...
  getCoachData() {
    this.appService.getAll('/admin/getcoaches')
    .subscribe(response => {
        ...
        if (response && response['data']) {
          ...
          //console.log(dat);
            let data = dat.map(value=>{
              return {
                "slno":value.Coach_ID,
                "coachname": value.Coach_Fname,
                "email": value.Coach_Email,
                "phone" : value.Coach_Phone,
  ...
  getcoach(id){
    ...
    this.router.navigate(['/admin/coachedit'],{ queryParams: { Coach_id: id }});
  ...
  changecoachstatus(id){
    ...
    this.appService.create('/admin/coachstatustoactive', Coach_id)
          .subscribe((response) => {
  ...
  changecoachstatus1(id){
    ...
    this.appService.create('/admin/coachstatustoinactive', Coach_id)
          .subscribe((response) => {
            if (response && response.isSuccess == true) {
              window.scrollTo(0, 0);
              this._showAlertMessage('alert-success', 'Mis à jour avec succés');
